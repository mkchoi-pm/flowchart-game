...
function initGame() {
    ...
    const shapesData = [
        ...
        {idx: '2', type: 'square', label: 'SQL을 잘 만들도록 질문을 다듬어요!'},
        {idx: '3', type: 'square', label: '데이터를 가져올 테이블을 찾아요!'},
        {idx: '4', type: 'square', label: '사용자 질문을 SQL 쿼리로 바꿔요!'},
        // {idx: '7', type: 'square', label: '데이터 추출 성공했는가?'},
        // {idx: '8', type: 'square', label: '재질문 요청하기'},
        {idx: '5', type: 'square', label: '가져온 데이터를 해석하여 답변을 만들어요!'},
    ];
    ...
    document.querySelectorAll('.blank-box').forEach(blank => {
      ...
      blank.addEventListener('drop', e => {
        ...
        // }
        // 맞는 도형이면 빈칸에 위치 이동
        const shapeEl = document.querySelector(`.shape[data-index="${shapeIdx}"]`);
        ...
        document.getElementById('progress').innerHTML = `<span>${filled} / ${total}</span>`;
      });
    });
